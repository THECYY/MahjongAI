...
class ChuanCardPlayAI(nn.Module):
    ...
    def __init__(self, h_dim, transformer_layers, table_dim, river_dim, c_dim, head, dropout, act, decoder_layers, player_count=4, only_self=False):
        super().__init__()
        ...
        if not only_self:
            ...
            self.table_card_mlp = mlp(2, act, table_dim, c_dim, c_dim, False, True)
            self.mlp = mlp(3, act, c_dim + h_dim, h_dim, h_dim)
    ...
    def forward(
        ...
        if not self.only_self:
            ...
            selfs = st_emb
            z = torch.concatenate([h_emb, selfs], dim=-1)
